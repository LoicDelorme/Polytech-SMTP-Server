...
public class WaitingForOtherRecipientsState extends State {
	 ...
	 */
	public WaitingForOtherRecipientsState(MailDropRequest mailDropRequest) {
	...
	public StateResult executeCommand(String receivedCommand, BufferedReader inputStream) {
		...
		if (receivedCommand.startsWith(RCPTTO.COMMAND_NAME)) {
			...
			nextState = this;
		} else if (receivedCommand.startsWith(DATA.COMMAND_NAME)) {
			commandResult = COMMANDS.get(DATA.COMMAND_NAME).execute(this.mailDropRequest, receivedCommand);

			message = commandResult.toString();
			nextState = commandResult.wasWellExecuted() ? new WaitingForDataState(commandResult.getMailDropRequest()) : this;
